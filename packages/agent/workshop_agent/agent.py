...
class Api:
    def __init__(self, base_url: str, token: str, device_serial: str):
        if not base_url.startswith(HTTP_SCHEMES):
            raise ValueError("Adres API musi zaczynać się od http:// lub https://")
    ...
    def _call(self, method: str, path: str, body: dict | None = None):
        request = Request(  # noqa: S310 — scheme restricted to http(s) in __init__
            f"{self.base_url}/integrations/fiscal/agent/jobs/{path}",
            data=json.dumps(body).encode() if body is not None else None,
            headers=self.headers,
            method=method,
        )
        with urlopen(request, timeout=30) as response:  # noqa: S310 — see __init__
...
def main(printer: Printer, config_path: Path, api_url: str | None, token: str | None) -> int:
    ...
    if not config.get("api_url") or not config.get("token"):
        ...
        return 2
    if not config["api_url"].startswith(HTTP_SCHEMES):
        log.error("Adres API musi zaczynać się od http:// lub https://")
        return 2
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.touch(mode=0o600)
    config_path.chmod(0o600)  # the token is a credential; also tightens a file from older runs
